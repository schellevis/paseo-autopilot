...
Before paid delegation, inspect the repository and complete intake. Read [workflow.md](references/workflow.md) now and at each transition. Do not launch until the preset, routing policy, concurrency, permissions, acceptance criteria, and external/destructive boundaries are resolved.
...
Ask the user only about unresolved intake choices and material decisions. Routine corrections proceed autonomously. In unattended operation, never self-approve a material choice: persist the decision request, enter `AWAITING_USER`, and continue only independent authorized work.
