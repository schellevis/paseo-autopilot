...
- **Delegated author (optional):** the same task-fit rule as specification/architecture, since a delegated author drafts the same document. Delegation itself is optional; when used, prefer a different vendor/account scope from the reviewers where possible. The orchestrator scans and adjudicates the draft as untrusted content before adopting it.
- **Spec/plan review:** strong critical reasoning from a different underlying family/vendor/account scope than the author when possible; a mid-tier model often suffices for routine review of bounded scope. Across each configured review set, prefer genuinely different underlying vendors/model families and account scopes when available within approved routing and budget. Varying only thinking depth on one family is not review diversity; a different transport or account alone does not provide a different reasoning family. Record the set's actual diversity and any unavailable or budget/routing-constrained alternatives in the brief and the document's resolution. Preserve configured review counts and independent review; this preference does not authorize extra reviewers or unapproved models.
...
- **Integration and debugging:** reliable repository-wide reasoning and tool use. Put the most reliable, fast option that fits the approved budget and routing on critical-path integration; escalate effort for a stated subtle or cross-file problem. The controller retains coordination; integration tasks use existing builder/repairer roles, not a new routing role. Use cheaper or experimental alternatives only on parallel, low-risk, independently verifiable assignments, not as an unproven critical-path dependency.
...
Read the actual available provider/account usage meter before and after every wave and whenever an agent stalls, following `paseo-runtime.md`. Record timestamped scope and remaining/reset information, or explicit unavailability, in the brief. Account for the controller's own usage when it shares a vendor/account window with workers. A new session or another model in that same scope does not replenish the window. Use observed headroom to order approved fallbacks and avoid launching work into a known exhausted scope; never infer quota exhaustion from silence or a missing meter.
