...
Check the real usage meter before and after each wave and whenever the checklist identifies a stall or an idle agent without its expected report. Use a discovered provider/account meter under the same authentication scope as the affected agents. Use the meter candidate that matches each vendor/account scope, only if the installed CLI supports it and returns actual meter data: for a Claude account `claude -p "/usage"`, and for a Codex/OpenAI account `codex usage` or the discovered equivalent. Do not treat generated text or an unsupported-command response as a reading. If that operation is unsupported, use another discovered meter, or record `unavailable` with the reason. Record timestamp, scope, remaining/reset information when supplied, and the orchestrator's shared usage in `00-brief.md`; never invent a reset time or remaining balance. Meter unavailability alone neither proves exhaustion nor authorizes failover. Finish notifications do not replace this procedure. Read these meters during intake discovery as well, before the step-3 proposal table is built, so measured per-account headroom is available when defaults are chosen; record the reading in `00-brief.md` the same way.
