...
The orchestrator authors `01-spec.md` and `03-plan.md` itself and remains the sole writer of both and of `run.json`.
...
The documentation reconciliation step reuses existing run-state fields without adding new ones. The documentation task is declared in the plan's last wave but its assessment/apply attempts execute post-VERIFY (deferred; only `COMPLETE` enforces all tasks completed). It carries two-plus builder attempts (assessment, optional apply). Task status moves `planned` → `blocked` (while the user decision is pending) → `completed`. The pre-human audit is a `verifier` attempt with a `doc-audit-<task-id>` assignment recorded under `reviews/verification/` and additional to the configured code verifiers. The user choice is a `material` decision (`kind: material`, category `scope-outcome`) whose artifact holds the verbatim per-file proposal. The outcome is named in the build report ("Material discoveries"), `05-verification-resolution.md`, and `06-final.md`. This adds no new `run.json` field, `kind`, phase, or status.
