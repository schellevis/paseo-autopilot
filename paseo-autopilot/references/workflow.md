...
- During every wait-loop status poll, inspect activity/logs for current provider usage signals as specified in `paseo-runtime.md`, even when an agent is still running. Read the actual available provider/account meter before and after each wave and whenever an agent stalls, accounting for the controller's shared consumption. Confirmed limit evidence enters the usage-interruption branch in the same poll; terminal status and a report are not prerequisites. Quoted task content and generic retry text alone do not establish a limit. Meter unavailability or silence alone does not authorize usage failover.
